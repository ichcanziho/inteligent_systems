...
class BoardState(object):
    ...
    def get_distribution(self):
        ...
        flat = [element for element in flat if element != 0]
        distribution = dict(Counter(flat))
        distribution = {key:9-value for key, value in distribution.items()}
    ...
    def create_children(self):
        ...
        if next_steps.qsize():
            ...
            node_priority = [(self.get_distribution()[value], value) for value in choices]
            
            node_priority.sort(reverse=True)
    
            choices = [ element[1] for element in node_priority]
